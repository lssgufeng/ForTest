...
HDIB CCeXDib::Create(DWORD dwWidth, DWORD dwHeight, WORD wBitcounts)
{
...
/*
    ...
    if (wBitcounts <= 1)			wBitcounts = 1;
    else if (wBitcounts <= 4)	wBitcounts = 4;
    else if (wBitcounts <= 8)	wBitcounts = 8;
    else				        wBitcounts = 24;
...
*/
    m_dwLineWidth = WIDTHBYTES(wBitcounts * dwWidth);
    ...
    m_bi.biBitCount = wBitcounts;    // from parameter
...
{
	Create(src->GetWidth(), src->GetHeight(), src->GetBitcounts());
	if (m_hDib) memcpy(m_hDib, src->m_hDib, GetSize());
} // End of Clone

WORD CCeXDib::GetBitcounts()
{
